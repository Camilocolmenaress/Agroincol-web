...
interface FAQAccordionProps { faqs: Array<{ question: string; answer: string }>; }
      ...
      {faqs.map((faq, index) => (
        <div key={index}>
          <button onClick={() => setOpenIndex(openIndex === index ? null : index)} className="w-full flex items-center justify-between py-5 text-left">
            ...
            <ChevronDown size={20} className={`text-brand-orange shrink-0 transition-transform duration-200 ${openIndex === index ? 'rotate-180' : ''}`} />
          </button>
